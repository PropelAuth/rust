...
use crate::{apis::ResponseContent, models::create_org_saml_connection_link_response};
...
/// struct for passing parameters to the method [`create_org_saml_connection_link`]
#[derive(Clone, Debug, Default)]
pub struct CreateOrgSamlConnectionLinkParams {
    pub org_id: String,
...
/// struct for typed errors of method [`create_org_saml_connection_link`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateOrgSamlConnectionLinkError {
    Status401(serde_json::Value),
    Status404(serde_json::Value),
    UnknownValue(serde_json::Value),
}
...
pub async fn create_org_saml_connection_link(configuration: &configuration::Configuration, params: CreateOrgSamlConnectionLinkParams) -> Result<crate::models::CreateOrgSamlConnectionLinkResponse, Error<CreateOrgSamlConnectionLinkError>> {
    let local_var_configuration = configuration;

    // unbox the parameters
    let org_id = params.org_id;
    let create_org_saml_connection_link_body = {
        let mut request_body = HashMap::new();
        if let Some(param_value) = params.expires_in_seconds {
            request_body.insert("expires_in_seconds".to_string(), param_value.to_string());
        }
        request_body
    };
    ...
    let local_var_uri_str = format!("{}/api/backend/v1/org/{org_id}/create_saml_connection_link", local_var_configuration.base_path, org_id=crate::apis::urlencode(org_id));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
    local_var_req_builder = local_var_req_builder.json(&create_org_saml_connection_link_body);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CreateOrgSamlConnectionLinkError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
