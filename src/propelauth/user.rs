...
use crate::propelauth::errors::{
    ...
    MigrateUserError, UpdatePasswordError, UpdateUserEmailError, UpdateUserMetadataError, ClearPasswordError
};
...
impl UserService<'_> {
    ...
    pub async fn invite_user_to_org(
        &self,
        invite_user_to_org_request: crate::models::InviteUserToOrgRequest,
    ) -> Result<(), InviteUserToOrgError> {
        if !is_valid_id(&invite_user_to_org_request.org_id) {
            return Err(InviteUserToOrgError::NotFound);
        }

        let params = crate::apis::user_service_api::InviteUserToOrgParams {
            invite_user_to_org_request,
        };
        crate::apis::user_service_api::invite_user_to_org(&self.config, params)
            .await
            .map_err(|err| {
                map_autogenerated_error(
                    err,
                    ...
                    |status_code, err_entity| match (status_code.as_u16(), err_entity) {
                        (
                            _,
                            Some(
                                crate::apis::user_service_api::InviteUserToOrgError::Status400(
                                    bad_request,
                                ),
                            ),
    ...
    pub async fn enable_user_can_create_orgs(&self, user_id: String) -> Result<(), ErrorsWithNotFound> {
        if !is_valid_id(&user_id) {
            return Err(ErrorsWithNotFound::NotFound);
        }
    ...
    pub async fn disable_user_can_create_orgs(&self, user_id: String) -> Result<(), ErrorsWithNotFound> {
        if !is_valid_id(&user_id) {
            return Err(ErrorsWithNotFound::NotFound);
        }
