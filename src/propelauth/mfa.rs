...
use crate::apis::mfa_service_api::{StepUpMfaError, VerifySmsChallengeParams, VerifyStepUpGrantParams};
use crate::apis::Error;
use crate::models::{SendSmsCodeResponse, VerifySmsChallengeResponse, VerifyStepUpGrantResponse};
use crate::propelauth::errors::{VerifyStepUpGrantError, VerifyStepUpTotpChallengeError, SendSmsCodeError, VerifySmsChallengeError};
...
impl MfaService<'_> {
    ...
    pub async fn verify_step_up_grant(
        ...
        let result =
            crate::apis::mfa_service_api::verify_step_up_grant(&self.config, params).await;
            ...
            Err(Error::ResponseError(response)) => {
                ...
                if let Ok(error_json) = error_response {
                    if let Some(error_code) = error_json.get("error_code").and_then(|v| v.as_str())
                    {
                        match error_code {
                            "invalid_request_fields" => {
                                if let Some(field_to_errors) =
                                    error_json.get("field_to_errors").and_then(|v| v.as_object())
                                {
                                    if let Some(grant_error) =
                                        field_to_errors.get("grant").and_then(|v| v.as_str())
                                    {
                                        if grant_error == "grant_not_found" {
                                            return Ok(VerifyStepUpGrantResponse { success: false });
                                        }
                                    }
                                }
                            ...
                            "feature_gated" => {
                                return Err(VerifyStepUpGrantError::FeatureGated)
                            }
                            _ => {}
                        }
                    }
    ...
    pub async fn send_sms_mfa_code(
        ...
        let result =
            crate::apis::mfa_service_api::send_sms_mfa_code(&self.config, params).await;
            ...
            Err(Error::ResponseError(response)) => {
                ...
                if let Ok(error_json) = error_response {
                    if let Some(error_code) = error_json.get("error_code").and_then(|v| v.as_str())
                    {
                        match error_code {
                            "user_not_found" => {
                                return Err(SendSmsCodeError::UserNotFound)
                            }
                            "mfa_not_enabled" => {
                                return Err(SendSmsCodeError::MfaNotEnabled)
                            }
                            "invalid_request_fields" => {
                                return Err(SendSmsCodeError::BadRequest(
                                    response.content,
                                ))
                            }
                            "feature_gated" => {
                                return Err(SendSmsCodeError::FeatureGated)
                            }
                            _ => {}
                        }
                    }
    ...
    pub async fn verify_sms_challenge(
        ...
        let result =
            crate::apis::mfa_service_api::verify_sms_challenge(&self.config, params).await;
            ...
            Err(Error::ResponseError(response)) => {
                ...
                if let Ok(error_json) = error_response {
                    if let Some(error_code) = error_json.get("error_code").and_then(|v| v.as_str())
                    {
                        match error_code {
                            "user_not_found" => {
                                return Err(VerifySmsChallengeError::UserNotFound)
                            }
                            "mfa_not_enabled" => {
                                return Err(VerifySmsChallengeError::MfaNotEnabled)
                            }
                            "invalid_request_fields" => {
                                return Err(VerifySmsChallengeError::BadRequest(
                                    response.content,
                                ))
                            }
                            "feature_gated" => {
                                return Err(VerifySmsChallengeError::FeatureGated)
                            }
                            _ => {}
                        }
                    }
